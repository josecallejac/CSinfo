...
const initDataTable = async () => {
    ...
    dataTable = $("#datatable-players").DataTable(dataTableOptions);
...
const listPlayers = async () => {
    try {
        const response = await fetch("http://127.0.0.1:8000/hltv/list_players");
        const data = await response.json();

        let content = ``;
        data.players.forEach((player, stats) => {
            content += `
                <tr>
                    <td>${stats + 1}</td>
        ...
        tableBody_players.innerHTML = content;
    } catch (ex) {
        // alert(ex);
    }
};
